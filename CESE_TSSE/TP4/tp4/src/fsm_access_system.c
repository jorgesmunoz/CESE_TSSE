...
debounce_button_t button1; //Botón open en modo manual
debounce_button_t button2; //Botón close en modo manual
debounce_button_t button3; //Botón para modo manual
debounce_button_t button4; //Botón para modo consola
...
void fsm_systemInit( system_t  * pSystem ){
	...
	//Inicializo máquina de estados finitos debounce para TEC1, TEC2, TEC3 y TEC4
...
void fsm_systemUpdate( system_t  * pSystem ){
	...
	else if(button4.keyPressed == TRUE){
...
static void system_console(system_t * pSystem){
	...
	if(system_uart_ReadByte( UART, &message )){
		if(message == 'S'){
			pSystem->state = OPEN;
